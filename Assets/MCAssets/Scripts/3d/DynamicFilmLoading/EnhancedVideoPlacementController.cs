...
public class PolygonZoneManagerEditor : Editor
{
    ...
    {
        // The rest of the editor code can remain unchanged
        // This part doesn't directly impact the zone assignment issue
    }

    private void OnSceneGUI()
    {
        // The scene GUI code can remain unchanged
        // This part doesn't directly impact the zone assignment issue
    }
}
...
public class PolygonZoneManager : MonoBehaviour
{
    ...
    private EnhancedVideoPlacementController placementController;
    ...
    private void FindRequiredComponents()
    {
        ...
        {
            placementController = FindObjectOfType<EnhancedVideoPlacementController>();
        }
    }
    ...
    // Find a zone by name with case-insensitive comparison
    public PolygonZone FindZoneByName(string zoneName)
    {
        if (string.IsNullOrEmpty(zoneName))
        {
            Debug.LogWarning("FindZoneByName called with empty zone name");
            return null;
        }

        // Do a case-insensitive comparison for more reliable matching
        PolygonZone zone = zones.FirstOrDefault(z =>
            string.Equals(z.Name, zoneName, StringComparison.OrdinalIgnoreCase));

        if (zone == null && debugMode)
        {
            Debug.LogWarning($"Zone '{zoneName}' not found. Available zones: {string.Join(", ", zones.Select(z => z.Name))}");
        }
