...
/////////// ************** INSERT AND UPDATE AGE
	echo "<br> ---------------------------------<br>Start of insert/update for #age <br> ---------------------------------<br>";
$ageCheckquery = "SELECT * from userhabits where habit_id ='" . $ageID . "' and user_id = '" . $userid . "';"; 
echo "ageCheckquery: " . $ageCheckquery . "<br>";
$agecheck = mysqli_query($conn, $ageCheckquery) or die("2:age check query failed");

if(mysqli_num_rows($agecheck) >0)
{
	echo "3: habit already exists";
	$updateagevalue = "UPDATE userhabits SET amount = '" . $age . "' WHERE user_id = '" . $userid . "' AND habit_id = '" . $ageID . "';"; 
echo "updateagevalue:" . $updateagevalue . "<br>";

mysqli_query($conn, $updateagevalue) or die("4: update to age failed "); //insert data or error for failure
